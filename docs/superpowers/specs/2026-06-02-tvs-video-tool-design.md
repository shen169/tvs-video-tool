...
用户在半自动模式下，Stage 1-2 自动跑完，Stage 3 选风格需交互，Stage 5 分镜预览需确认，Stage 6 视频生成后下载。
...
```
pending → fetching → image_gen → style_wait → script_gen → preview_wait → video_gen → done
                       ↓             ↓                        ↓
                      failed      (等待用户选风格)        (等待用户确认分镜)
```
