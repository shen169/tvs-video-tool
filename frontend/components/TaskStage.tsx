    ...
    switch (stage) {
      ...
      case "ref_image":
        return (
          <div className="space-y-6 animate-in animate-in-1">
            ...
            <StageShell icon="🖼️" title="Reference Image" subtitle="AI-generated product hero shot">
              {uploaded_ref_image ? (
                <img src={uploaded_ref_image} className="w-full max-w-sm rounded-2xl" />
              ) : ref_image_url && !ref_image_url.startsWith("__") ? (
                <img src={ref_image_url} className="w-full max-w-sm rounded-2xl shadow-xl shadow-amber-500/5" />
              ) : (
      ...
      case "script_gen":
        return (
          <div className="space-y-6 animate-in animate-in-1">
            {showProduct && <ProductAnalysis info={product_info} />}
            <StageShell icon="📝" title="Writing Script & Storyboard" subtitle="AI is crafting 6-shot sequences for each platform">
      ...
      case "video_gen":
        return (
          <div className="space-y-6 animate-in animate-in-1">
            <StageShell icon="🎬" title="Generating Video" subtitle="Seedance 2.0 is rendering your product video">
              <div className="flex items-center gap-4 mt-2">
                <Spinner />
                <div>
      ...
      case "done":
        return video_urls
          ...
          : <StageShell icon="✅" title="Complete" subtitle=""><p className="text-sm text-zinc-500">Done</p></StageShell>;

      case "failed":
        return (
          <StageShell icon="✕" title="Task Failed">
